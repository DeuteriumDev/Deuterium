...
comment on table {{ private_schema }}.dt_config is E'Private [key: string], [value: string] config used internally by deuterium';

insert into {{ private_schema }}.dt_config values ('configuration', '{{ configuration }}');
insert into {{ private_schema }}.dt_config values ('version', '1');
